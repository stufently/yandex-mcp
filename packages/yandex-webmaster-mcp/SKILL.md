...
Сервер `yandex-webmaster` — 31 тул поверх `https://api.webmaster.yandex.net/v4`.
...
Причина по каждому URL живёт в **`get-search-events-samples`**: у записей с
`event: REMOVED_FROM_SEARCH` заполнено поле `excluded_url_status`, а тул дополнительно
считает сводку `exclusion_reasons` по странице выдачи.
