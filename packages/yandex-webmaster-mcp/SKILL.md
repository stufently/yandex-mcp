...
4. **`get-external-links`** / **`get-broken-internal-links`** — внешняя ссылочная масса и
   собственные битые ссылки (у последнего есть `count` — общее число, а не только примеры).
5. **`get-popular-queries`** — запросы, по которым сайт показывается и получает клики
...
- **`get-excluded-pages`** — то, что нужно в 9 случаях из 10: список исключённых страниц
  с причиной по каждой. `limit` считает ИСКЛЮЧЁННЫЕ СТРАНИЦЫ, а не события: серверного
  фильтра по типу события у ресурса нет, поэтому тул сам обходит смешанный поток
  страницами по 100, не больше `max_requests` вызовов. Не набрал `limit` — вернёт
  `exhausted: false` и `next_offset`, с которого продолжить. Поле
  `total_events_both_types` — это события ОБОИХ типов, а НЕ число исключённых страниц:
  сколько всего исключено, ресурс не сообщает.
